...
# Mark root user as expired so the password must be changed before anything else can be done
...
# check arguments for an option that would cause mysqld to stop return true if there is one
_mysql_want_help() {
	local arg
	for arg; do
		case "$arg" in
